...
from kholmatov_sdk.client import Client


class TestClient(unittest.TestCase):

    def setUp(self):
        self.client = Client(api_key='your-api-key')
        self.endpoint = 'https://the-one-api.dev/v2/'
        self.headers = {'Authorization': 'Bearer your-api-key'}
        mock_data_movies_path = os.path.join(os.path.dirname(__file__), 'mock_data', 'movies.json')
        with open(mock_data_movies_path, 'r') as f:
            self.mock_data_movies = json.load(f)

        mock_data_quotes_path = os.path.join(os.path.dirname(__file__), 'mock_data', 'quotes.json')
    ...
    def test_get_movie(self, mock_get):
        ...
        response = self.client.get_movie('5cd95395de30eff6ebccde56')
    ...
    def test_get_movies_with_budget_less_than_100(self, mock_get):
        mock_get.return_value.json.return_value = self.mock_data_movies
        response = self.client.get_movies(budgetInMillions=('<', 100))
    ...
    def test_get_movies_with_awards_greater_than_0(self, mock_get):
        mock_get.return_value.json.return_value = self.mock_data_movies
        response = self.client.get_movies(academyAwardWins=('>', 0))
    ...
    def test_get_movies_with_runtime_greater_than_or_equal_to_160(self, mock_get):
        mock_get.return_value.json.return_value = self.mock_data_movies
        response = self.client.get_movies(runtimeInMinutes=('>=', 160))
    ...
    def test_get_movies_by_sorting(self, mock_get):
        mock_get.return_value.json.return_value = self.mock_data_movies
        response = self.client.get_movies(sort='name:asc')
    ...
    def test_get_movie_by_filtering(self, mock_get):
        mock_get.return_value.json.return_value = self.mock_data_movies
        response = self.client.get_movies(name='/Ring/i')
    ...
    def test_get_movie_quotes(self, mock_get):
        ...
        response = self.client.get_movie_quotes('5cd95395de30eff6ebccde5d')
    ...
    def test_get_quote(self, mock_get):
        ...
        response = self.client.get_quote('5cd96e05de30eff6ebccebd0')
    ...
    def test_get_quotes_by_pagination(self, mock_get):
        mock_get.return_value.json.return_value = self.mock_data_quotes
        response = self.client.get_quotes(page=2, limit=10)
